...
async function stampLocalCard(ctx, contactId, originalVCard, serverPerson) {
  ...
  await ctx.markServer(ctx.targetID, contactId, "modified_by_server");
...
async function runPullPass(ctx) {
  ...
  for (const person of people) {
    ...
    if (!existing) {
      ...
      await ctx.markServer(targetID, null, "added_by_server");
      ...
      const vCard = mapper.personToVCard(person);
      await ctx.markServer(targetID, existing.id, "modified_by_server");
  ...
  for (const [resourceName, entry] of byResourceName) {
    ...
    await ctx.markServer(targetID, entry.id, "deleted_by_server");
...
async function runPushDeletePass(ctx, userEntries, serverResourceNames) {
  ...
  for (const entry of deletions) {
    ...
    try {
      ...
      console.warn(
        `[google-4-tbsync] push.delete ${resourceName} failed:`,
        err?.message ?? err
      );
...
async function runGroupPullPass(ctx, byResourceName, memberMap) {
  ...
  for (const [listId, rn] of listIdToResourceName) {
    ...
    if (list) {
      await ctx.markServer(targetID, listId, "deleted_by_server");
  ...
  for (const [resourceName, entry] of mappings) {
    ...
    for (const id of expectedIds) {
      if (!currentIds.has(id)) {
        ...
        await ctx.markServer(listId, id, "modified_by_server");
...
async function runGroupPushDeletePass(ctx, userEntries) {
  ...
  for (const entry of deletions) {
    ...
    try {
      ...
      console.warn(
        `[google-4-tbsync] push.group.delete ${mapping.resourceName} failed:`,
        err?.message ?? err
      );
    }
  }
